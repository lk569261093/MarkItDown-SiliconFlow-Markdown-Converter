...
- 已默认包含 `markitdown[docx,pdf]` 依赖，可直接处理 DOCX/PDF 文件。
...
# 如果你是旧环境升级，单独补齐 DOCX/PDF 依赖
pip install -i https://pypi.tuna.tsinghua.edu.cn/simple -U "markitdown[docx,pdf]"
...
## Streamlit Cloud 部署排错（DOCX/PDF 依赖）

如果云端出现 `DocxConverter/PdfConverter MissingDependencyException`，按下面顺序处理：

1. 确认部署分支已包含最新 `requirements.txt` 与 `runtime.txt`。
2. 在 Streamlit Cloud 中执行 **Reboot app** 和 **Clear cache** 后重新部署。
3. 查看构建日志，确认已安装 `markitdown[docx,pdf]`、`pdfplumber`、`pdfminer.six`。
